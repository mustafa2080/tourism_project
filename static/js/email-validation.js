...
class EmailValidator {
  ...
  validate(email) {
    ...
    if (!this.patterns.username.test(username)) {
      return {
        isValid: false,
        message: 'Username can only contain letters, numbers, dots, hyphens, or underscores',
    ...
    if (!this.patterns.domain.test(domain)) {
      return {
        isValid: false,
        message: 'Domain must be valid and include a top-level domain (e.g., .com)',
        code: 'invalid_domain'
      };
    }

    // Check for consecutive special characters
    if (this.patterns.consecutiveSpecials.test(email)) {
      return {
        isValid: false,
        message: 'Email cannot contain consecutive special characters (e.g., .., --, __)',
        code: 'consecutive_specials'
      };
    }

    // Check for invalid characters
    if (this.patterns.invalidChars.test(email)) {
      return {
        isValid: false,
    ...
    // Check if TLD is in the disallowed list
    if (this.config.disallowedTLDs.includes(tld)) {
      return {
        isValid: false,
        message: `Invalid top-level domain: ".${tld}" is not allowed. Use a complete domain like ".com"`,
        code: 'disallowed_tld'
      };
    }
    
    // Check if TLD is in the allowed list
    if (!this.config.validTLDs.includes(tld)) {
      return {
        isValid: false,
        message: `Invalid top-level domain: ".${tld}" is not recognized`,
        code: 'invalid_tld'
      };
    }
