...
class BookingUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_queryset(self):
        ...
        adult_subtotal = regular_price * num_adults
        child_subtotal = regular_price * Decimal('0.5') * num_children  # Children at half price
        subtotal = adult_subtotal + child_subtotal

        # Calculate discount if applicable
        discount_amount = 0
        if tour.has_discount and tour.discount_price is not None:
            # Calculate the discount per person
            discount_per_adult = regular_price - tour.discount_price
        ...
        # Check if this is an AJAX request (from the integrated payment flow)
        if self.request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            print(f"Processing AJAX request for booking ID: {self.object.id}")
            # Return JSON response with booking ID for payment processing
            # Return JSON response with booking ID and payment details for payment processing
            response_data = {
                'success': True,
                'booking_id': self.object.id,
                'total_price': str(self.object.total_price), # Send as string for precision
                'currency_code': self.request.session.get('currency_code', 'USD'),
                'message': _("Booking created successfully! Proceeding to payment...")
            }
            ...
            # Standard form submission (fallback - might be deprecated if AJAX is always used)
            messages.success(self.request, _("Booking created successfully! Please proceed to payment."))

            # Redirect to booking confirmation with steps (starting at step 2: Payment)
            confirmation_url = reverse('booking:booking_confirmation_steps', kwargs={'pk': self.object.id})
